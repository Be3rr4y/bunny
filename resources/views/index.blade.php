...
{{--have cut away from navigation tothe topor skeleton of the html document--}}
{{--This is called rendering turning the code into html that can be displayed on a browser
 @extends tells Laravel: "before doing anything else, first go and collect all of MY @section content (this child page's pieces) 
and remember them"      THEN Laravel goes and processes layouts.main (the skeleton)
 as it hits each @yield(...), it reaches into the section content
    it collected from me a moment ago, and drops it into that exact spot
only once every yield has been filled in does Laravel produce
   ONE final combined HTML output — never two separate HTML outputs,
and never a "half-finished" HTML page shown at any point--}}
{{--tell laravel start capturing content that will be needed later during the rendering process

this file is supossed to enable us centralise the common sections on all pages that in case
 we need to make any changes instead of going to the different individual pages 
we just come here--}}
<!---->
<!---->
